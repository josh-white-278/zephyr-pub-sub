				...
				struct pub_sub_subscriber *subscriber, uint16_t msg_id)
{
	...
	pub_sub_msg_init(&delayable_msg->msg, msg_id, PUB_SUB_ALLOC_ID_DELAYABLE_MSG);
}
...
void pub_sub_free_delayable_msg(const void *msg)
{
	...
	struct pub_sub_delayable_msg *delayable_msg =
		CONTAINER_OF(msg, struct pub_sub_delayable_msg, msg);
...
static void timer_timeout(struct k_timer *timer)
{
	K_SPINLOCK(&g_spin_lock)
	{
		...
		while ((delayable_msg != NULL) &&
			...
			struct pub_sub_delayable_msg *tmp_msg = delayable_msg;
