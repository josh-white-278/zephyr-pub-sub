...
void pub_sub_delayable_msg_init(void *msg, struct pub_sub_subscriber *subscriber, uint16_t msg_id)
{
	...
	struct pub_sub_msg_delayable *delayable_msg =
		CONTAINER_OF(msg, struct pub_sub_msg_delayable, pub_sub_msg.msg);
	sys_dnode_init(&delayable_msg->node);
...
int pub_sub_delayable_msg_start(const void *msg, k_timeout_t timeout)
{
	...
	K_SPINLOCK(&g_spin_lock)
	{
		...
		struct pub_sub_msg_delayable *insert_msg =
			SYS_DLIST_PEEK_HEAD_CONTAINER(&g_wait_list, insert_msg, node);
		ret = (pub_sub_msg_get_ref_cnt(msg) == 0) ? 0 : -EBUSY;
		// If the message is linked then we need to remove it
		if (sys_dnode_is_linked(&delayable_msg->node)) {
			...
			if (!sys_timepoint_expired(delayable_msg->end_time) &&
			    (sys_timepoint_cmp(new_end_time, delayable_msg->end_time) > 0)) {
				insert_msg = SYS_DLIST_PEEK_NEXT_CONTAINER(&g_wait_list,
									   delayable_msg, node);
				sys_dlist_remove(&delayable_msg->node);
			} else {
				sys_dlist_remove(&delayable_msg->node);
		...
		delayable_msg->end_time = new_end_time;
		...
		if (sys_dlist_is_empty(&g_expired_list)) {
			...
			if ((head_msg == delayable_msg) ||
			    (sys_timepoint_cmp(head_msg->end_time, g_current_end_time) != 0)) {
				g_current_end_time = head_msg->end_time;
				k_timer_start(&g_msg_timer,
					      sys_timepoint_timeout(head_msg->end_time), K_NO_WAIT);
			}
		}
	}
	return ret;
...
int pub_sub_delayable_msg_abort(const void *msg)
{
	...
	K_SPINLOCK(&g_spin_lock)
	{
		...
		if (sys_dnode_is_linked(&delayable_msg->node)) {
			sys_dlist_remove(&delayable_msg->node);
			struct pub_sub_msg_delayable *head_msg =
				SYS_DLIST_PEEK_HEAD_CONTAINER(&g_wait_list, head_msg, node);
			...
			if ((head_msg == NULL) && expired_list_empty) {
				...
				k_timer_stop(&g_msg_timer);
				   ...
				   (sys_timepoint_cmp(head_msg->end_time, g_current_end_time) >
				    0)) {
...
bool pub_sub_delayable_msg_is_active(const void *msg)
{
	...
	K_SPINLOCK(&g_spin_lock)
	{
		ret = sys_dnode_is_linked(&delayable_msg->node);
	}
	return ret;
}
			     ...
			     struct pub_sub_msg_delayable *next_msg)
{
	...
	while ((next_msg != NULL) &&
	       (sys_timepoint_cmp(next_msg->end_time, msg_to_add->end_time) <= 0)) {
		next_msg = SYS_DLIST_PEEK_NEXT_CONTAINER(&g_wait_list, next_msg, node);
	}
	if (next_msg == NULL) {
		sys_dlist_append(&g_wait_list, &msg_to_add->node);
	} else {
		sys_dlist_insert(&next_msg->node, &msg_to_add->node);
	}
}
...
static void timer_timeout(struct k_timer *timer)
{
	K_SPINLOCK(&g_spin_lock)
	{
		...
		struct pub_sub_msg_delayable *delayable_msg =
			SYS_DLIST_PEEK_HEAD_CONTAINER(&g_expired_list, delayable_msg, node);
		while (delayable_msg != NULL) {
			void *msg = delayable_msg->pub_sub_msg.msg;
			struct pub_sub_msg_delayable *next_msg =
				SYS_DLIST_PEEK_NEXT_CONTAINER(&g_expired_list, delayable_msg, node);
		...
		delayable_msg = SYS_DLIST_PEEK_HEAD_CONTAINER(&g_wait_list, delayable_msg, node);
		while ((delayable_msg != NULL) && sys_timepoint_expired(delayable_msg->end_time)) {
			struct pub_sub_msg_delayable *tmp_msg = delayable_msg;
			void *msg = tmp_msg->pub_sub_msg.msg;
			delayable_msg = SYS_DLIST_PEEK_NEXT_CONTAINER(&g_wait_list, tmp_msg, node);
			sys_dlist_remove(&tmp_msg->node);
			if (pub_sub_msg_get_ref_cnt(msg) == 0) {
				pub_sub_acquire_msg(msg);
				pub_sub_publish_to_subscriber(tmp_msg->subscriber, msg);
			} else {
				sys_dlist_append(&g_expired_list, &tmp_msg->node);
			}
		}
		...
		if (delayable_msg != NULL) {
			new_timeout = sys_timepoint_timeout(delayable_msg->end_time);
			g_current_end_time = delayable_msg->end_time;
		}
		if (!sys_dlist_is_empty(&g_expired_list)) {
