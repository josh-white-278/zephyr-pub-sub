...
static hsm_status_t test_top_state_fn(struct hsm *hsm, uint16_t msg_id, const void *msg);
static hsm_status_t test_parent_0_far_state_fn(struct hsm *hsm, uint16_t msg_id, const void *msg);
static hsm_status_t test_parent_1_far_state_fn(struct hsm *hsm, uint16_t msg_id, const void *msg);
static hsm_status_t test_far_state_fn(struct hsm *hsm, uint16_t msg_id, const void *msg);
static hsm_status_t test_sub_state_fn(struct hsm *hsm, uint16_t msg_id, const void *msg);
static hsm_status_t test_start_state_fn(struct hsm *hsm, uint16_t msg_id, const void *msg);
static hsm_status_t test_start_sibling_state_fn(struct hsm *hsm, uint16_t msg_id, const void *msg);
static hsm_status_t test_start_child_state_fn(struct hsm *hsm, uint16_t msg_id, const void *msg);
static hsm_status_t test_start_child_of_child_state_fn(struct hsm *hsm, uint16_t msg_id,
						       const void *msg);
static hsm_status_t test_diff_top_state_fn(struct hsm *hsm, uint16_t msg_id, const void *msg);
static hsm_status_t test_diff_child_state_fn(struct hsm *hsm, uint16_t msg_id, const void *msg);
static hsm_status_t test_trans_on_entry_to_start_state_fn(struct hsm *hsm, uint16_t msg_id,
							  const void *msg);
static hsm_status_t test_trans_on_entry_child_state_fn(struct hsm *hsm, uint16_t msg_id,
						       const void *msg);
static hsm_status_t test_trans_on_entry_to_trans_on_entry_child_state_fn(struct hsm *hsm,
									 uint16_t msg_id,
									 const void *msg);
static hsm_status_t test_recursive_state_fn(struct hsm *hsm, uint16_t msg_id, const void *msg);

struct test_hsm test_hsm;

static const HSM_STATE_DEFINE(test_top_state, test_top_state_fn, NULL);
static const HSM_STATE_DEFINE(test_parent_0_far_state, test_parent_0_far_state_fn, &test_top_state);
static const HSM_STATE_DEFINE(test_parent_1_far_state, test_parent_1_far_state_fn,
			      &test_parent_0_far_state);
static const HSM_STATE_DEFINE(test_far_state, test_far_state_fn, &test_parent_1_far_state);
static const HSM_STATE_DEFINE(test_sub_state, test_sub_state_fn, &test_top_state);
static const HSM_STATE_DEFINE(test_start_state, test_start_state_fn, &test_sub_state);
static const HSM_STATE_DEFINE(test_start_sibling_state, test_start_sibling_state_fn,
			      &test_sub_state);
static const HSM_STATE_DEFINE(test_start_child_state, test_start_child_state_fn, &test_start_state);
static const HSM_STATE_DEFINE(test_start_child_of_child_state, test_start_child_of_child_state_fn,
			      &test_start_child_state);
static const HSM_STATE_DEFINE(test_diff_top_state, test_diff_top_state_fn, NULL);
static const HSM_STATE_DEFINE(test_diff_child_state, test_diff_child_state_fn,
			      &test_diff_top_state);
static const HSM_STATE_DEFINE(test_trans_on_entry_to_start_state,
			      test_trans_on_entry_to_start_state_fn, &test_top_state);
static const HSM_STATE_DEFINE(test_trans_on_entry_child_state, test_trans_on_entry_child_state_fn,
			      &test_trans_on_entry_to_start_state);
static const HSM_STATE_DEFINE(test_trans_on_entry_to_trans_on_entry_child_state,
			      test_trans_on_entry_to_trans_on_entry_child_state_fn,
			      &test_top_state);
// Make this state its own parent to test maximum state depth
static const HSM_STATE_DEFINE(test_recursive_state, test_recursive_state_fn, &test_recursive_state);
...
static void publish_transition_state(struct hsm *hsm, uint16_t msg_id,
				     const struct hsm_state *dest_state)
{
...
ZTEST(hsm_basic, test_start)
{
	...
	zassert_equal(test_hsm.msg_rx_data[0].state_fn, test_top_state_fn);
	zassert_equal(test_hsm.msg_rx_data[0].msg_id, HSM_MSG_ID_ENTRY);
	zassert_equal(test_hsm.msg_rx_data[1].state_fn, test_sub_state_fn);
	zassert_equal(test_hsm.msg_rx_data[1].msg_id, HSM_MSG_ID_ENTRY);
	zassert_equal(test_hsm.msg_rx_data[2].state_fn, test_start_state_fn);
...
ZTEST(hsm_basic, test_parent_state_rx)
{
	...
	zassert_equal(test_hsm.num_msg_received, 1);
	zassert_equal(test_hsm.msg_rx_data[0].state_fn, test_sub_state_fn);
...
ZTEST(hsm_basic, test_transition_to_child)
{
	...
	zassert_equal(test_hsm.msg_rx_data[0].state_fn, test_start_state_fn);
	zassert_equal(test_hsm.msg_rx_data[0].msg_id, MSG_ID_TEST_TRANSITION_START_STATE);
	zassert_equal(test_hsm.msg_rx_data[1].state_fn, test_start_child_state_fn);
	zassert_equal(test_hsm.msg_rx_data[1].msg_id, HSM_MSG_ID_ENTRY);

	test_hsm.num_msg_received = 0;
	publish_msg(&test_hsm.hsm, MSG_ID_TEST_GET_CURRENT_STATE);
	zassert_equal(test_hsm.num_msg_received, 1);
	zassert_equal(test_hsm.msg_rx_data[0].state_fn, test_start_child_state_fn);
	zassert_equal(test_hsm.msg_rx_data[0].msg_id, MSG_ID_TEST_GET_CURRENT_STATE);
}
...
ZTEST(hsm_basic, test_transition_to_child_of_child)
{
	...
	zassert_equal(test_hsm.msg_rx_data[1].state_fn, test_start_child_state_fn);
	zassert_equal(test_hsm.msg_rx_data[1].msg_id, HSM_MSG_ID_ENTRY);
	zassert_equal(test_hsm.msg_rx_data[2].state_fn, test_start_child_of_child_state_fn);
	zassert_equal(test_hsm.msg_rx_data[2].msg_id, HSM_MSG_ID_ENTRY);

	test_hsm.num_msg_received = 0;
	publish_msg(&test_hsm.hsm, MSG_ID_TEST_GET_CURRENT_STATE);
	zassert_equal(test_hsm.num_msg_received, 1);
	zassert_equal(test_hsm.msg_rx_data[0].state_fn, test_start_child_of_child_state_fn);
	zassert_equal(test_hsm.msg_rx_data[0].msg_id, MSG_ID_TEST_GET_CURRENT_STATE);
}
...
ZTEST(hsm_basic, test_transition_to_sibling)
{
	...
	zassert_equal(test_hsm.msg_rx_data[2].state_fn, test_start_sibling_state_fn);
	zassert_equal(test_hsm.msg_rx_data[2].msg_id, HSM_MSG_ID_ENTRY);

	test_hsm.num_msg_received = 0;
	publish_msg(&test_hsm.hsm, MSG_ID_TEST_GET_CURRENT_STATE);
	zassert_equal(test_hsm.num_msg_received, 1);
	zassert_equal(test_hsm.msg_rx_data[0].state_fn, test_start_sibling_state_fn);
	zassert_equal(test_hsm.msg_rx_data[0].msg_id, MSG_ID_TEST_GET_CURRENT_STATE);
}
...
ZTEST(hsm_basic, test_transition_to_far_state)
{
	...
	zassert_equal(test_hsm.msg_rx_data[1].state_fn, test_start_state_fn);
	zassert_equal(test_hsm.msg_rx_data[1].msg_id, HSM_MSG_ID_EXIT);
	zassert_equal(test_hsm.msg_rx_data[2].state_fn, test_sub_state_fn);
	zassert_equal(test_hsm.msg_rx_data[2].msg_id, HSM_MSG_ID_EXIT);
	zassert_equal(test_hsm.msg_rx_data[3].state_fn, test_parent_0_far_state_fn);
	zassert_equal(test_hsm.msg_rx_data[3].msg_id, HSM_MSG_ID_ENTRY);
	zassert_equal(test_hsm.msg_rx_data[4].state_fn, test_parent_1_far_state_fn);
	zassert_equal(test_hsm.msg_rx_data[4].msg_id, HSM_MSG_ID_ENTRY);
	zassert_equal(test_hsm.msg_rx_data[5].state_fn, test_far_state_fn);
	zassert_equal(test_hsm.msg_rx_data[5].msg_id, HSM_MSG_ID_ENTRY);

	test_hsm.num_msg_received = 0;
	publish_msg(&test_hsm.hsm, MSG_ID_TEST_GET_CURRENT_STATE);
	zassert_equal(test_hsm.num_msg_received, 1);
	zassert_equal(test_hsm.msg_rx_data[0].state_fn, test_far_state_fn);
	zassert_equal(test_hsm.msg_rx_data[0].msg_id, MSG_ID_TEST_GET_CURRENT_STATE);
}
...
ZTEST(hsm_basic, test_transition_to_different_hsm)
{
	...
	zassert_equal(test_hsm.msg_rx_data[4].state_fn, test_diff_top_state_fn);
	zassert_equal(test_hsm.msg_rx_data[4].msg_id, HSM_MSG_ID_ENTRY);
	zassert_equal(test_hsm.msg_rx_data[5].state_fn, test_diff_child_state_fn);
	zassert_equal(test_hsm.msg_rx_data[5].msg_id, HSM_MSG_ID_ENTRY);

	test_hsm.num_msg_received = 0;
	publish_msg(&test_hsm.hsm, MSG_ID_TEST_GET_CURRENT_STATE);
	zassert_equal(test_hsm.num_msg_received, 1);
	zassert_equal(test_hsm.msg_rx_data[0].state_fn, test_diff_child_state_fn);
	zassert_equal(test_hsm.msg_rx_data[0].msg_id, MSG_ID_TEST_GET_CURRENT_STATE);
}
...
ZTEST(hsm_basic, test_transition_on_entry_simple)
{
	...
	zassert_equal(test_hsm.msg_rx_data[2].state_fn, test_sub_state_fn);
	zassert_equal(test_hsm.msg_rx_data[2].msg_id, HSM_MSG_ID_EXIT);
	zassert_equal(test_hsm.msg_rx_data[3].state_fn, test_trans_on_entry_to_start_state_fn);
	zassert_equal(test_hsm.msg_rx_data[3].msg_id, HSM_MSG_ID_ENTRY);
	zassert_equal(test_hsm.msg_rx_data[4].state_fn, test_trans_on_entry_to_start_state_fn);
	zassert_equal(test_hsm.msg_rx_data[4].msg_id, HSM_MSG_ID_EXIT);
	zassert_equal(test_hsm.msg_rx_data[5].state_fn, test_sub_state_fn);
	zassert_equal(test_hsm.msg_rx_data[5].msg_id, HSM_MSG_ID_ENTRY);
	zassert_equal(test_hsm.msg_rx_data[6].state_fn, test_start_state_fn);
...
{
	publish_transition_state(&test_hsm.hsm, MSG_ID_TEST_TRANSITION_START_STATE,
				 &test_trans_on_entry_to_trans_on_entry_child_state);
	// We expect 9 msgs:
	// start_state: MSG_ID_TEST_TRANSITION_START_STATE
	// start_state, sub_state: EXIT
	...
	zassert_equal(test_hsm.msg_rx_data[4].state_fn,
		      test_trans_on_entry_to_trans_on_entry_child_state_fn);
	zassert_equal(test_hsm.msg_rx_data[4].msg_id, HSM_MSG_ID_EXIT);
	zassert_equal(test_hsm.msg_rx_data[5].state_fn, test_trans_on_entry_to_start_state_fn);
	zassert_equal(test_hsm.msg_rx_data[5].msg_id, HSM_MSG_ID_ENTRY);
	zassert_equal(test_hsm.msg_rx_data[6].state_fn, test_trans_on_entry_to_start_state_fn);
	zassert_equal(test_hsm.msg_rx_data[6].msg_id, HSM_MSG_ID_EXIT);
	zassert_equal(test_hsm.msg_rx_data[7].state_fn, test_sub_state_fn);
	zassert_equal(test_hsm.msg_rx_data[7].msg_id, HSM_MSG_ID_ENTRY);
	zassert_equal(test_hsm.msg_rx_data[8].state_fn, test_start_state_fn);
...
{
	hsm_start(&test_hsm.hsm, &test_trans_on_entry_to_start_state);
	...
	zassert_equal(test_hsm.msg_rx_data[0].state_fn, test_top_state_fn);
	zassert_equal(test_hsm.msg_rx_data[0].msg_id, HSM_MSG_ID_ENTRY);
	zassert_equal(test_hsm.msg_rx_data[1].state_fn, test_trans_on_entry_to_start_state_fn);
	zassert_equal(test_hsm.msg_rx_data[1].msg_id, HSM_MSG_ID_ENTRY);
	zassert_equal(test_hsm.msg_rx_data[2].state_fn, test_trans_on_entry_to_start_state_fn);
	zassert_equal(test_hsm.msg_rx_data[2].msg_id, HSM_MSG_ID_EXIT);
	zassert_equal(test_hsm.msg_rx_data[3].state_fn, test_sub_state_fn);
	zassert_equal(test_hsm.msg_rx_data[3].msg_id, HSM_MSG_ID_ENTRY);
	zassert_equal(test_hsm.msg_rx_data[4].state_fn, test_start_state_fn);
	zassert_equal(test_hsm.msg_rx_data[4].msg_id, HSM_MSG_ID_ENTRY);
...
{
	hsm_start(&test_hsm.hsm, &test_trans_on_entry_to_trans_on_entry_child_state);
...
ZTEST(hsm_basic, test_state_depth)
{
	...
	// Expect CONFIG_HSM_MAX_NESTED_STATES entry messages for each parent + 1 for new state
	zassert_equal(test_hsm.num_msg_received, CONFIG_HSM_MAX_NESTED_STATES + 1);
	for (size_t i = 0; i < CONFIG_HSM_MAX_NESTED_STATES; i++) {
		zassert_equal(test_hsm.msg_rx_data[i].state_fn, test_recursive_state_fn);
	...
	// CONFIG_HSM_MAX_NESTED_STATES + 1 of test_recursive_state: EXIT
	// top_state, sub_state, start_state: ENTRY
	zassert_equal(test_hsm.num_msg_received, 1 + CONFIG_HSM_MAX_NESTED_STATES + 1 + 3);
	zassert_equal(test_hsm.msg_rx_data[0].state_fn, test_recursive_state_fn);
	zassert_equal(test_hsm.msg_rx_data[0].msg_id, MSG_ID_TEST_TRANSITION_START_STATE);
	for (size_t i = 0; i < CONFIG_HSM_MAX_NESTED_STATES + 1; i++) {
		zassert_equal(test_hsm.msg_rx_data[i + 1].state_fn, test_recursive_state_fn);
	...
	zassert_equal(test_hsm.msg_rx_data[CONFIG_HSM_MAX_NESTED_STATES + 3].msg_id,
		      HSM_MSG_ID_ENTRY);
	zassert_equal(test_hsm.msg_rx_data[CONFIG_HSM_MAX_NESTED_STATES + 4].state_fn,
		      test_start_state_fn);
	zassert_equal(test_hsm.msg_rx_data[CONFIG_HSM_MAX_NESTED_STATES + 4].msg_id,
		      HSM_MSG_ID_ENTRY);
...
static hsm_status_t test_far_state_fn(struct hsm *hsm, uint16_t msg_id, const void *msg)
{
	...
	case HSM_MSG_ID_EXIT:
	case MSG_ID_TEST_GET_CURRENT_STATE:
		test_hsm->msg_rx_data[test_hsm->num_msg_received].state_fn = test_far_state_fn;
		test_hsm->msg_rx_data[test_hsm->num_msg_received++].msg_id = msg_id;
		return HSM_CONSUMED();
	default:
		return HSM_PASS();
...
static hsm_status_t test_sub_state_fn(struct hsm *hsm, uint16_t msg_id, const void *msg)
{
	...
	case HSM_MSG_ID_ENTRY:
	case HSM_MSG_ID_EXIT:
		test_hsm->msg_rx_data[test_hsm->num_msg_received].state_fn = test_sub_state_fn;
		test_hsm->msg_rx_data[test_hsm->num_msg_received++].msg_id = msg_id;
						       ...
						       const void *msg)
{
	...
	case MSG_ID_TEST_GET_CURRENT_STATE:
		test_hsm->msg_rx_data[test_hsm->num_msg_received].state_fn =
			test_start_child_of_child_state_fn;
		test_hsm->msg_rx_data[test_hsm->num_msg_received++].msg_id = msg_id;
		return HSM_CONSUMED();
	default:
		return HSM_PASS();
...
static hsm_status_t test_diff_top_state_fn(struct hsm *hsm, uint16_t msg_id, const void *msg)
{
	...
	case HSM_MSG_ID_EXIT:
	case MSG_ID_TEST_GET_CURRENT_STATE:
		test_hsm->msg_rx_data[test_hsm->num_msg_received].state_fn = test_diff_top_state_fn;
		test_hsm->msg_rx_data[test_hsm->num_msg_received++].msg_id = msg_id;
		return HSM_CONSUMED();
	default:
		return HSM_PASS();
							  ...
							  const void *msg)
{
	...
	case HSM_MSG_ID_ENTRY:
		test_hsm->msg_rx_data[test_hsm->num_msg_received].state_fn =
			test_trans_on_entry_to_start_state_fn;
		test_hsm->msg_rx_data[test_hsm->num_msg_received++].msg_id = msg_id;
						       ...
						       const void *msg)
{
	...
	case MSG_ID_TEST_GET_CURRENT_STATE:
		test_hsm->msg_rx_data[test_hsm->num_msg_received].state_fn =
			test_trans_on_entry_child_state_fn;
		test_hsm->msg_rx_data[test_hsm->num_msg_received++].msg_id = msg_id;
		return HSM_CONSUMED();
	default:
		return HSM_PASS();
